...
import { asBaseComponent, Button, ButtonProps, Colors, Shadows, View } from "react-native-ui-lib";

export const ActionButton = function ({ icon, ...buttonProps }) {
  return (
      <Button {...buttonProps} >
        { icon }
      </Button>
        ...
        <View row centerV paddingH-20={!centered} style={[this.styles.container, style]} {...others}>
          {_.map(actions, (action, i) => (
            <View key={i} flex {...this.getAlignment(i)}>
              <ActionButton link size={Button.sizes.medium} $textPrimary {...action} />
